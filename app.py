...
button { border-radius: 12px !important; font-weight: bold; box-shadow: 0px 0px 20px rgba(124, 58, 237, 0.6); }
button:hover { transform: scale(1.03); transition: 0.2s; }
textarea, input { border-radius: 10px !important; }
"""

def run(issue_title, issue_desc, files, code, exp_severity, exp_component, exp_fix):
    time.sleep(0.5)
    obs = {
...
💻 **Code**
<pre style="background:#0d1117;padding:12px;border-radius:10px;"><code>{safe_code}</code></pre>
""",
...
""")
    ...
    desc_input = gr.Textbox(label="Bug Description", value="Navbar link redirects to wrong page")
    ...
    with gr.Row():
        expected_severity = gr.Dropdown(choices=["LOW", "MEDIUM", "HIGH"], label="Expected Severity", value="LOW")
        expected_component = gr.Dropdown(choices=["UI", "BACKEND", "DATABASE", "API"], label="Expected Component", value="UI")
    expected_fix = gr.Textbox(label="Expected Fix (optional)", placeholder="e.g. Use parameterized queries")
    ...
    btn.click(
        run,
        inputs=[title_input, desc_input, files_input, code_input, expected_severity, expected_component, expected_fix],
        outputs=[obs_box, action_box, reward_box]
    )
    gr.Markdown("---\n💡 Built with ❤️ for intelligent bug triage systems")

# ================= MOUNT FASTAPI INTO GRADIO (single port) ================= #
